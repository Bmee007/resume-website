# Borina Keo — Verified Facts & Response Data
...
- Designs agent harnesses: execution loops, tool routing, error recovery, observability hooks
- Builds domain skills: supply chain data retrieval, ERP write-back, warehouse query tools
- Architects multi-step workflows: plan → act → observe → reflect loops
- Agent frameworks and patterns: tool-use agents, ReAct, Plan-and-Execute, multi-agent orchestration

## LLM Memory Research

- Episodic memory: storing and retrieving past agent interactions and decisions
- Semantic memory: knowledge graph integration for domain facts (SKUs, supplier data, SLAs)
- Working memory: context window management for long-horizon planning tasks
- Applying memory patterns to supply chain AI agents that need operational history

## Agent Security

- Threat modeling for autonomous agents: prompt injection, tool output poisoning, cross-agent trust escalation
- Authorization boundaries: least-privilege action scoping, confirmation gates for destructive operations
- Defensive harness patterns: input sanitization, output validation, privilege containment
- Red teaming agentic systems to discover and close attack vectors before deployment

## Domain Expertise: Supply Chain, Healthcare, Retail

- Supply chain: demand forecasting, inventory optimization, procurement automation, 3PL integration
